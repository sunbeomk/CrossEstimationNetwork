...
class ConstraintLayer(Layer):
    ...
    def __init__(self, n_item, psi_sq_fixed_indices, lambda_positive_indices, **kwargs):
        ...
        self.lambda_positive_indices = tf.constant(lambda_positive_indices, dtype=tf.int32)

    def call(self, item_params):
        lambdas = item_params[:, :self.n_item]
        psi_sqs_raw = item_params[:, self.n_item:2*self.n_item] # Raw values from Dense layer
        gammas = item_params[:, 2*self.n_item:]

        # CRITICAL FIX, PART 1: Apply softplus to all psi_sq values first...
        psi_sqs_positive = tf.math.softplus(psi_sqs_raw)

        # ...THEN overwrite the marker items with a hard 1.0 constraint.
        ...
        # Constrain marker lambdas (this part is correct)
...
class CEN:
    def __init__(
        ...
        self.inp_size_person_net, self.inp_size_item_net = inp_size_person_net, inp_size_item_net
    ...
    def _build_IRT_net(self):
        ...
        def compute_prob(inputs):
            ...
            psi_sq_i = tf.gather_nd(psi_sq_all, tf.stack([b_idx, i_i], 1))
            psi_sq_k = tf.gather_nd(psi_sq_all, tf.stack([b_idx, i_k], 1))
    ...
    def _preprocess_data(self):
        ...
        self.X_person_net = np.repeat(self.res_mat, self.n_comps, axis=0)
    ...
    def param_est(self):
        eta_est = self.person_net.predict(self.res_mat, verbose=0)
        ...
        all_psi_sq_raw = item_params[:, self.n_item:2*self.n_item]
        ...
        for c_idx in range(self.n_comps):
            ...
            psi_sq_s[i] += all_psi_sq_raw[c_idx, i]
            psi_sq_s[k] += all_psi_sq_raw[c_idx, k]
